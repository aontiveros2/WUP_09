...
The p-value is so small that it shows this model is an excellent fit. 

```{r}
anova(fullup,best.model)
```
  
The p-value is 0.879 which means that there is really no difference between using the model that has all the data versus the model that uses only 2 variables.  
...
Based on the automated selection of variables there is no real difference in using a model with two variables versus using one with all of the variables. This was identified through a graphical depiction of the adjusted R-squared as well as the p-values. In conclusion comparing the p-values of the two variable models the p-values were so small that i reject the null hypothesis that these models are beneficial in predicting weight of fish. The p-values associated with the anovas were large and therefore result in a failure to reject the null hypothesis, there is no difference between using the model with two variables versus the fullup models.   
